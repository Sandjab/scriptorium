...
Chaque document est sous `themes/<slug>/dist/`. Le corpus compte **62 thèmes** (64 documents),
...
- Phase 2 — workflow multi-agents + `SKILL.md` des skills `monograph` et `frugalmonograph` : ✅
  ...
  Build), document unique « best-of » + widgets pilotés par concept. Éprouvé sur **61 thèmes**
  hors APO et publié via GitHub Pages.
- Skill `leanmonograph` (« vérifier d'abord, écrire une fois » + lint déterministe) : ✅
  éprouvé — les **22 monographies les plus récentes** en sont issues.
