...
const VOICE = `VOIX D'AUTEUR (un seul auteur pour tout le document) : (1) ARC — suis le fil rouge fourni ; chaque section ouvre en reliant à ce qui précède (une phrase de transition suffit, pas de résumé) et referme en tendant vers la suite. (2) PREMIÈRE OCCURRENCE GLOBALE — un concept, un sigle, un système (BLINK, CRF…) ne se présente qu'UNE fois dans tout le document, à sa première apparition ; ensuite emploie-le nu. Consulte le récapitulatif « déjà posé » fourni : ce qui y figure est déjà introduit. (3) RYTHME — alterne phrases longues et courtes ; jamais deux paragraphes consécutifs construits sur le même moule ; pas de prose-liste (une suite de « X fait Y. Z fait W. ») ; pas plus d'UN chiffre secondaire par phrase — hiérarchise : le fait porteur en phrase principale, les chiffres d'appoint en incise ou dans la carte de fait. (4) REGISTRE — essai technique français : précis, direct, sans emphase ni remplissage (« il est important de noter », « dans le monde de », « véritable »… bannis). (5) HONNÊTETÉ — n'affirme QUE ce que la matière fournie établit ; les controverses restent des controverses.`;
...
const authorTldrGlossPrompt = (sectionsBrief) => [
  ...
  ``,
  `2) ${themeDir}/tldr.json — { "these": "<la thèse du document en 1 phrase>", "part1": ["…","…"], "part2": ["…","…"] }. part1 et part2 = 2-4 puces chacune (idées clés ; part1 = principe, part2 = garanties/limites). Ce fichier alimente le RÉSUMÉ (abstract) en tête du document — sois complet. FONDE chaque puce sur un claim vérifié de knowledge.json (jamais sur ta mémoire).`,
  ``,
