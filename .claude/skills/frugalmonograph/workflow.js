export const meta = {
  ...
  description: "Variante FRUGALE de monograph : même pipeline vérifié (Sweep→Plan→Extract→Verify→Réconcilie→Author→Widgets→Compose→Build), mais modèles moins chers sur la recherche/vérification, council réduit (2 jurés) et plafonds durs — pour produire une monographie HTML à coût réduit.",
  ...
  phases: [
    ...
    { title: 'Réconcilie', detail: 'aligne la prose des sections sur les claims corrigés', model: 'sonnet' },
  ...
  if (toReconcile.length === 0) {
    ...
    const results = (await parallel(toReconcile.map(({ s, corr }) => () =>
      A(reconcilePrompt(s.section.heading, s.section.prose, corr),
        { schema: S_RECONCILE, model: M_RESEARCH, phase: 'Réconcilie', label: `reconcile:${s.section.id}` })
...
if (!Array.isArray(loadedWidgets)) {
  ...
  await A(
    ...
    { schema: { type:'object', additionalProperties:false, required:['written'], properties:{ written:{type:'boolean'} } },
      model: M_IO, phase: 'Widgets', label: 'write:sections' }
  );
}
